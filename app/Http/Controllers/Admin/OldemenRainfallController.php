...
use App\Models\PredictionRainfall;

class OldemenRainfallController extends Controller
{
    ...
    {
        $monthlies = Rainfall::orderBy("year")->get();

        foreach ($monthlies as $m) {
            $m->oldeman = OldemanService::get($m->bb, $m->bk);
        }

        $predictions = PredictionRainfall::all();

        foreach ($predictions as $p) {
            $p->oldeman = OldemanService::get($p->bb, $p->bk);
        }

        return view("pages.admin.oldemen-rainfall.index", compact("monthlies", "predictions"));
    }

    public function predictionForm()
    {
        return renderToJson("pages.admin.oldemen-rainfall.prediction");
    }

    public function deletePredictionForm()
    {
        $predictions = PredictionRainfall::all();
        return renderToJson("pages.admin.oldemen-rainfall.delete-prediction", compact("predictions"));
    }

    public function deletePrediction(Request $request)
    {
        try {
            PredictionRainfall::where("year", $request->year)->delete();
            return back();
        } catch (\Exception $e) {
            dd($e);
        }
    ...
    {
        $year = $request->year;
        $prediction = PredictionRainfall::where("year", $request->year)->count();
        if ($prediction) {
            ...
            PredictionRainfall::where("year", $year)->update([
                "year" => $request->year,
                "jan" => $this->jan,
                "feb" => $this->feb,
                "mar" => $this->mar,
    ...
    {
        $prediction = PredictionRainfall::where("year", $year)->first() ?? null;
        $rainfall   = Rainfall::where("year", $year)->first() ?? null;
    ...
    private function updateMonthTypePrediction($year)
    {
        ...
        $rainfall = PredictionRainfall::where("year", $year)->first();
